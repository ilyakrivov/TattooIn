...
# Конфигурация
TOKEN = "8145217179:AAFj5N1gL6i405T9RTHih5NiHznMqOxLVMM"
SPREADSHEET_ID = "1uC3WoJ2eUOsi_zQ5AohIpHZQtfhkWykqTjApx6Uzaic"
SCOPES = ["https://www.googleapis.com/auth/spreadsheets"]

# Инициализация Google Sheets
try:
    creds = ServiceAccountCredentials.from_json_keyfile_name("tattoo-income-bot-e446e5e6ab6d.json", SCOPES)
...
async def update_sheet(message: types.Message, column: int, value: str):
    """Обновляем или создаем запись мастера с суммированием"""
    try:
        user = message.from_user.full_name  # Используем полное имя вместо логина
        search_query = user.lower().strip()  # Имя пользователя в нижнем регистре для поиска

        # Попробуем найти ячейку с запросом
        cell = None
        try:
...
async def process_category(message: types.Message, state: FSMContext):
    ...
    if message.text == "Клиент ухаживает сам":
        # Записываем 0 и завершаем процесс
        data = await state.get_data()
        user = message.from_user.full_name
        
        success1 = await update_sheet(message, COLUMNS[data['type']], data['amount'])
        success2 = await update_sheet(message, COLUMNS["Плёнка"], "0")  # Записываем 0 в плёнку
...
async def process_final(message: types.Message, state: FSMContext):
    try:
        ...
        success1 = await update_sheet(message, COLUMNS[data['type']], data['amount'])
        success2 = await update_sheet(message, COLUMNS[data['category']], message.text)
...
async def restart_process(message: types.Message, state: FSMContext):
    ...
    await start(message, state)  # Перезапускаем процесс с начала
